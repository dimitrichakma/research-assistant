...
- **TypeSafe (Jev) for judgment, Claude for generation.** Every place this project used to lean on regex or "ask an LLM and hope the reply parses" — classifying PDF blocks as figures vs. equations, matching a fuzzy section name to a real heading, reranking BM25 candidates, routing the supervisor's decisions — now asks a typed `Choice`/`Noul` question instead. Regex still does the cheap, deterministic parts (column-order sorting); TypeSafe only steps in where the decision is actually fuzzy. Measured, not assumed: the supervisor's jargon/analogy judgment on a real paper took 1.69s and **0 Claude tokens** through TypeSafe (it's a fully separate API call, never touching the BYOK model) vs. 6.81s and 38,406 tokens (~$0.08 for one call) when asked as a freeform "reason step by step" prompt to the main LLM instead — same answer either way, since the task is a narrow classification, not open-ended generation.
